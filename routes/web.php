...
Route::get('/panel/login', 'Auth\LoginController@AdminLogin')->name('panel.login');
Route::post('/panel/login', 'Auth\LoginController@AdminLogin')->name('panel.login.post');
Route::get("/login", 'Auth\LoginController@UserLogin')->name('login');
Route::post("/login", 'Auth\LoginController@UserLogin')->name('login.post');
Route::get('/login/cek_email/json', 'UserController@cek_email');
Route::get('/login/cek_nisn/json', 'UserController@cek_nisn');
Route::get('/login/cek_password/json', 'UserController@cek_password');
Route::get('/login/cek_password_nisn/json', 'UserController@cek_password_by_nisn');
...
Route::prefix('ppdb')->name('ppdb.')->group(function () {
  Route::get('/daftar', 'PPDBController@create')->name('create');
  Route::post('/daftar', 'PPDBController@store')->name('store');
});
...
Route::middleware(['auth'])->group(function () {
  Route::prefix('ppdb')->name('ppdb.')->group(function () {
    ...
    Route::get('/siswa-diterima', 'PPDBController@siswa_diterima')->name('siswa.diterima');
    Route::get('/{id}', 'PPDBController@show')->name('show');
    Route::get('/{id}/edit', 'PPDBController@edit')->name('edit');
    Route::patch('/{id}', 'PPDBController@update')->name('update');
    Route::delete('/{id}', 'PPDBController@destroy')->name('destroy');
    Route::get('/{id}/accept', 'PPDBController@accept_ppdb')->name('accept');
    Route::get('/{id}/reject', 'PPDBController@reject_ppdb')->name('reject');
    Route::post('/bulk/accept', 'PPDBController@bulk_accept_ppdb')->name('bulk.accept');
    Route::post('/bulk/reject', 'PPDBController@bulk_reject_ppdb')->name('bulk.reject');
  });
  ...
  Route::middleware(['guru'])->group(function () {
    ...
    Route::post('/ulangan/store2', 'UlanganController@store2')->name('ulangan.store2');
